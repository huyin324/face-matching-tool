...
> 实测（RTX 3060 Laptop / 1280x720@25fps / 6 秒素材 150 帧）：
> 实时 23.1 fps、墙钟 6.86s → 全速 **46.3 fps、墙钟 3.26s，约 2.0 倍**，两者截图产出完全相同。
...
| 播放倍速 | 1X | 仅实时分析时有效；全速模式不按播放节奏推进，该滑块自动禁用 |
...
产物位于 `dist/视频人脸检测比对工具V1.2.exe`，可脱离 Python 环境运行。
...
│   └── validate_v12.py    # V1.2 分析模式回归自测（UI 状态 + 实时/全速吞吐对比）
├── .gitignore
├── LICENSE                # MIT
└── README.md
```
